...
const createTweet = asyncHandler(async (req, res) => {
  ...
  if (!tweet) throw new ApiError(400, "Provide content to post as tweet");

  const tweetNew = await Tweet.create({
    content: tweet,
    owner: user._id,
  });
  if (!tweetNew) throw new ApiError(500, "Problem in creating tweet");
  return res
    .status(200)
    .json(new ApiResponse(200, { tweetNew }, "Tweet Created Successfully"));
});
...
const getUserTweets = asyncHandler(async (req, res) => {
  ...
  if (!tweets) throw new ApiError(400, "No Tweets by this user");
  return res.status(200).json(200, { tweets }, "These are user tweets");
});
